...
function jlptNumberToLevel(jlptNew: number): string | null {
...
export async function loadJlptKanji(
  levels: string[] = ['N5', 'N4', 'N3', 'N2']
): Promise<Kanji[]> {
  ...
  const raw: Record<string, KanjiDataEntry> = await response.json();
