...
function testFilterByLevels(): void {
...
function testCreateSessionDefaults(): void {
...
function testRevealToggle(): void {
...
function testMarkKnownUnknownAndAccuracy(): void {
  let state = createSession(ALL_KANJI);
...
function testSetLevelsChangesPool(): void {
...
function testSetLevelsPreservesStats(): void {
  let state = createSession(ALL_KANJI, { levels: ['N3'] });
  state = markKnown(state);
  state = markUnknown(state);
  assert.equal(state.stats.seen, 2);
...
function testAdvanceDoesNotChangeStats(): void {
  let state = createSession(ALL_KANJI);
...
function testNormalizeLevelPreferenceValidValues(): void {
  assert.equal(
...
function testNormalizeLevelPreferenceInvalidOrEmpty(): void {
  assert.equal(
...
function testNormalizeLevelPreferenceNonString(): void {
  assert.equal(
