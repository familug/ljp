...
- `index.html`
  ...
  - Loads `styles.css` and `src/main.js` (ES module; compiled from `src/main.ts`).
...
- `src/main.ts` (compiles to `main.js`)
  ...
  - On fetch failure (e.g. offline), falls back to the small sample set from `kanjiData.ts`.
  - Registers the PWA service worker via `registerSw(window)`.
- `src/data/kanjiCache.ts` (compiles to `kanjiCache.js`)
  ...
  - Used by `main.ts` and `drawMain.ts`; tests in `tests/kanjiCache.test.ts`.
- `src/core/kanjiData.ts` (compiles to `kanjiData.js`)
...
- `src/data/jlptSource.ts` (compiles to `jlptSource.js`)
...
- `src/data/exampleOverrides.ts` (compiles to `exampleOverrides.js`)
...
- `src/buildMeta.ts` (compiles to `buildMeta.js`; generated by `scripts/updateBuildMeta.ts` at build time)
  - Exports a small `BUILD_META` object `{ hash, datetimeIso }` baked at commit time.
  - `src/main.ts`, `src/kanaMain.ts`, and `src/drawMain.ts` read this and write a footer line like `Build <hash> · <ISO_DATETIME>` for quick version identification.
- `src/core/quizCore.ts` (functional core; compiles to `quizCore.js`)
...
- `src/shell/appShell.ts` (imperative shell; compiles to `appShell.js`)
...
- `src/kanaMain.ts` (compiles to `kanaMain.js`)
...
- **PWA (Progressive Web App)**:
  ...
  - `src/registerSw.ts` (compiles to `registerSw.js`): `registerSw(win)` registers the worker with scope at the app root (works from `/`, `/kana/`, `/draw/` and from GitHub Pages subpath). Called from `main.ts`, `kanaMain.ts`, and `drawMain.ts`.
  - Service workers require a secure context (HTTPS or localhost); the app works locally over `http://localhost:...`.
- `src/drawMain.ts` (compiles to `drawMain.js`)
...
- Tests cover:
  - `tests/core.test.ts`: `filterByLevels`, `createSession` defaults, reveal toggle, `markKnown`/`markUnknown`/`getAccuracy`, `setLevels`, `advance`, `normalizeLevelPreference`.
  - `tests/kanjiCache.test.ts`: `getCachedKanji` (missing, invalid, null storage), `setCachedKanji` (round-trip, ignores non-array).
- Command:
  - **Bun** (required): `bun tests/core.test.ts` (or `bun run test`). Use Bun for tests and scripts. Bun runs TypeScript directly.
...
---------------
- Tests:
  - With Bun (preferred): `bun tests/core.test.ts`.
  - With Node: `node tests/core.test.ts` (if Node supports `--experimental-strip-types` or if tests are compiled).
...
TypeScript build
----------------
- Source: TypeScript (`.ts`) in `src/`, `scripts/`, `tests/`.
- Build: `bun run build` or `npm run build` → runs `build:meta` then `tsc`.
- `tsc` compiles `src/**/*.ts` to `.js` in place; HTML and imports reference `.js`.
- Scripts (`scripts/*.ts`) and tests (`tests/*.ts`) are excluded from `tsc`; Bun runs them directly.
- Pre-commit hook runs `npm run build` and stages `src/`.

GitHub Pages / Deployment notes
-------------------------------
- Project is **fully static** after build. Run `npm run build` or `bun run build` before deploying.
- Files expected at repository root:
  - `index.html`, `styles.css`, `src/**` (`.ts` source and compiled `.js`), `tests/**`, `package.json`, `README.md`, `AGENTS.md`.
- GitHub Pages:
  - Settings → Pages → Source: "Deploy from a branch".
  - Choose main branch and root directory.
  - App loads at `https://<user>.github.io/<repo>/`.
- ES modules:
  - All imports use relative paths with `.js` extension (`./core/kanjiData.js`, `../core/quizCore.js`); TypeScript emits these as-is.
  - Keep that structure if you move files; Pages must serve the compiled `.js` files.

Extending the kanji data
------------------------
- To add more kanji (recommended for full N3/N2 coverage):
  - Append new objects to `KANJI_N3` or `KANJI_N2` in `src/core/kanjiData.ts`.
...
-----------------------
- For any **logic change**, work in `quizCore.ts` and tests:
  - Example: introduce spaced repetition, weighting unknown cards more heavily:
    ...
    - Update tests in `core.test.ts` to describe the new behavior explicitly.
...
- When adding new UI features:
  - Extend `index.html` for structure, `styles.css` for presentation, and `appShell.ts` for behavior.
  - Keep **business logic** in `quizCore.ts` so it can stay well-tested.
- For example, to add:
  - **Per-level accuracy**:
...
----------------------------------
- New kanji or levels?
  - Update `kanjiData.ts`, ensure `level` values are consistent, update any level-selection UI if needed.
- New quiz logic?
  - Implement in `quizCore.ts`, adapt tests, then update shell rendering.
- New UI feature?
  - Adjust `index.html` + `styles.css` minimally; wire events in `appShell.ts`.
...
- Whenever an AI agent (or human) makes a **non-trivial change** to:
  ...
  - Core behavior (`quizCore.ts`, data model, state shape, quiz logic),
